...
const { isDate, isNaN, isEmpty, isString } = require('lodash')

function objectToLineProtocol (points = []) {
  return points
    .map((currVal) => {
      console.log('InfluxDB : LINEPROTOCOL_TIMESTAMP_1 : ' + currVal.timestamp)
      ...
      else currVal.timestamp = toNanoDate(parseInt(currVal.timestamp, 10)).getNanoTime()

      console.log('InfluxDB : LINEPROTOCOL_TIMESTAMP : ' + currVal.timestamp)

      // Transform tags to comma-delimited key=value
      if (currVal.tags) {
        currVal.tags = (isString(currVal.tags) && currVal.tags) || Object.keys(currVal.tags).reduce((acc, currTag) => {
      ...
      // Transform fields
      let fields = (isString(currVal.fields) && currVal.fields) || Object.keys(currVal.fields).reduce((acc, currField, idx, arr) => {
